...
@after_method('apply_link')
def apply_moc(self):
    mocfiles = Utils.to_list(getattr(self, 'mocfiles', []))
    for mocfile in mocfiles:
        infile = self.path.find_resource(mocfile)
        if not infile:
            raise Errors.WafError('cannot find input file %s for processing' % mocfile)

        if infile.name.endswith('.cpp'):
            cxxfile = infile.parent.find_or_declare(infile.change_ext('.moc').name)
            self.create_task('moc', infile, cxxfile)
        elif infile.read().find('Q_OBJECT') > 0:
            cxxfile = infile.parent.find_or_declare('moc_' + infile.change_ext('.cpp').name)
            self.create_task('moc', infile, cxxfile)

            objfile = cxxfile.change_ext('.o')
            self.create_task('cxx', cxxfile, objfile)

            self.link_task.inputs.append(objfile)
